...
DESCRIBE producto;
SELECT nombre, precio, TRUNCATE(precio*1,2) FROM producto;
SELECT nombre, precio AS 'euros', TRUNCATE(precio*1,2) AS 'dolars' FROM producto;
...
SELECT * FROM producto p INNER JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE LOWER(f.nombre) LIKE '%hewlett%' OR f.nombre LIKE 'seagate' OR f.nombre LIKE 'asus';
SELECT * FROM producto p INNER JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre IN ('Hewlett-Packard', 'Asus', 'Seagate'); /*al IN no li agraden els % */
...
SELECT p.nombre FROM fabricante f LEFT JOIN producto p ON p.codigo_fabricante = f.codigo WHERE f.nombre LIKE 'Lenovo';
SELECT * FROM producto WHERE precio = (SELECT p.precio FROM fabricante f JOIN producto p ON p.codigo_fabricante = f.codigo WHERE f.nombre LIKE 'Lenovo' ORDER BY p.precio DESC LIMIT 1);
SELECT p.nombre FROM fabricante f JOIN producto p ON p.codigo_fabricante = f.codigo WHERE f.nombre LIKE 'Lenovo' ORDER BY p.precio DESC LIMIT 1;
SELECT p.nombre FROM fabricante f JOIN producto p ON p.codigo_fabricante = f.codigo WHERE f.nombre LIKE '%Hewlett%' ORDER BY p.precio ASC LIMIT 1;
SELECT * FROM producto WHERE precio >= (SELECT p.precio FROM fabricante f JOIN producto p ON p.codigo_fabricante = f.codigo WHERE f.nombre LIKE 'Lenovo' ORDER BY p.precio DESC LIMIT 1);
SELECT p.nombre, p.precio, f.nombre FROM fabricante f LEFT JOIN producto p ON p.codigo_fabricante = f.codigo WHERE f.nombre LIKE 'Asus' AND p.precio > (SELECT avg(precio) FROM producto);